---
title: "Project: Honeypot Per VLAN"
...
**1. Identified suitable honeypot addresses** - Selected an unused address on each subnet, outside the DHCP range, that would never be legitimately assigned to a device. The specific addresses are deliberately not documented here: a published honeypot is no longer a honeypot.

**2. Configured honeypot addresses in Unifi** - Created a honeypot entry for each of the four networks in the Unifi Threat Management settings.
